...
import { CriaMachosService, CreateCriaMachoDto, AnimalDto } from 'src/app/core/services/cria-machos.service';
...
export class CriasMachosComponent {
  ...
  private mapDtoToModel(dto: AnimalDto): CriaMacho {
    return {
      ...
      detalles: (dto as any).detalles ?? null,
    };
  }
  ...
  private cargarCrias() {
    ...
    // Prefer getAllAsView when available to avoid manual DTO mapping
    (this.criaService as any).getAllAsView?.().subscribe?.({
      next: (res: CriaView[]) => {
  ...
  private cargarMadres() {
    ...
    // Use paridaService.getAllAsView when available
    (this.paridaService as any).getAllAsView?.().subscribe?.({
      next: (res: any[]) => {
        this.madres = (res || []).map((p) => ({ id: p.id, numero: (p as any).numero ?? (p as any).numeroArete ?? '', nombre: (p as any).nombre ?? '' }));
      ...
      error: () => {
        ...
        this.paridaService.getAll().subscribe({
          next: (res) => {
            this.madres = (res || []).map((p) => ({ id: p.id, numero: (p as any).numeroArete ?? (p as any).numero ?? '', nombre: (p as any).nombre ?? '' }));
            this.isLoadingMadres.set(false);
            this.form.get('madreId')?.enable({ emitEvent: false });
          },
          error: () => {
