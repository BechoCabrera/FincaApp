-- Migration script (safe): add missing columns to AnimalMovimientos table
-- This version DOES NOT change columns to NOT NULL. Use when you prefer to keep columns nullable.
...
-- 5) Populate FincaOrigenId/FincaDestinoId with Guid.Empty where null (optional)
UPDATE dbo.AnimalMovimientos
...
-- 6) Keep columns nullable (safer). If later you want NOT NULL, run a separate script after verifying data.
...
COMMIT;
PRINT 'Safe migration script completed.';
GO
